...
class Read extends Conn {
    ...
    public function ExeR($Campo, $Tabela, $Termos = null, $ParseString = null) {
        if (!empty($ParseString)):
            parse_str($ParseString, $this->Places);
        endif;
    ...
    public function ExeRowCount($Tabela, $ParseString = null) {
        if (!empty($ParseString)):
            parse_str($ParseString, $this->Places);
        endif;

        $this->Select = "SELECT COUNT(*) FROM {$Tabela}";
        $this->Execute();
    }
    ...
    public function ExeReadDist($Campo, $Tabela, $Termos = null, $ParseString = null) {
        if (!empty($ParseString)):
            parse_str($ParseString, $this->Places);
        endif;

        $this->Select = "SELECT DISTINCT {$Campo} FROM {$Tabela} {$Termos}";
        $this->Execute();
    }
    ...
    private function Execute() {
        ...
        try {
            ...
            $this->Result = null;
            Ops("<b>Erro ao Ler:</b> {$e->getMessage()}", $e->getCode());
        }
    }
